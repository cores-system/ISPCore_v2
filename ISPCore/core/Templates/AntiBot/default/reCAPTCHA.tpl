        ...
        else
        {
            ...
            timer = setInterval(function(){
                ...
                if(key){
                    ...
                    $.post('{isp:CoreApiUrl}/check/recaptcha', { recaptchaKey: key, HourCacheToUser: {isp:HourCacheToUser}, hash: '{isp:HashToreCAPTCHA}' }, function (data) {
                        var json = JSON.parse(JSON.stringify(data));
                        if (json.result) {
                            var date = new Date(new Date().getTime() + (60 * 1000) * 60 * json.hourToCookie);
                            document.cookie = 'isp.ValidCookie='+json.cookie+'; path=/; expires=' + date.toUTCString();
                            location.reload();
                        }
                        else {
